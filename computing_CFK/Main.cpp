...
////////
/// Added to output the list of generators, lists of gradings, and differentials

	outFile<<"List of generators :  \n[";
	for (int a = 0; a< KFC.Generators.size()-1; a = a+1){
		outFile<< KFC.Generators[a].Name << ", ";
	}
	outFile<< KFC.Generators[KFC.Generators.size()-1].Name << "]\n\n";

	outFile<<"Alexander gradings :  \n[";
	for (int a = 0; a< KFC.Generators.size()-1; a = a+1){
		outFile<< KFC.Generators[a].Alexander << ", ";
	}
	outFile<< KFC.Generators[KFC.Generators.size()-1].Alexander << "]\n\n";

	outFile<<"Maslov gradings :  \n[";
	for (int a = 0; a< KFC.Generators.size()-1; a = a+1){
		outFile<< KFC.Generators[a].Maslov << ", ";
	}
	outFile<< KFC.Generators[KFC.Generators.size()-1].Maslov << "]\n\n";

	cout<< "Differential:";
	cout<< typeid(KFC.Differential[0]).name()<<"\n\n";

	outFile<<"Differential :  \n[";
	for (int a = 0; a< KFC.Differential.size()-1; a = a+1){
		outFile<< "("<<KFC.Differential[a].StartingGen << ",";
		outFile<< KFC.Differential[a].EndingGen << ",";
		outFile<< KFC.Differential[a].Coeff <<"), ";
	}
	outFile<<"("<<KFC.Differential[KFC.Differential.size()-1].StartingGen << ",";
	outFile<< KFC.Differential[KFC.Differential.size()-1].EndingGen << ",";
	outFile<< KFC.Differential[KFC.Differential.size()-1].Coeff << ")]\n\n";
